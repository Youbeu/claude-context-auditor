import { AuditResult, AnomalyReport } from '../types/index.js';
...
export class TerminalReporter {
   ...
   */
  static render(audit: AuditResult): string {
    const lines: string[] = [];
    ...
    let barColor = GREEN;
    if (sum.overheadPercentage > 30) barColor = RED;
    else if (sum.overheadPercentage > 15) barColor = YELLOW;
    ...
    for (const server of audit.mcpServers) {
      ...
      lines.push(
        `│ ${statusIcon} ${name.padEnd(20)} │ ${server.tokenCost.toLocaleString().padStart(10)} tk │ ${pct.padStart(10)} % │ ${CostCalculator.formatCurrency(cost).padStart(10)} │`
      );
    }
    ...
    for (const skill of audit.injectedSkills) {
      ...
      lines.push(
        `│ ${icon} ${name.padEnd(19)} │ ${skill.tokens.toLocaleString().padStart(10)} tk │ ${pct.padStart(10)} % │ ${CostCalculator.formatCurrency(cost).padStart(10)} │`
      );
    }
    ...
    for (const rule of audit.rules) {
      ...
      lines.push(
        `│ ${BLUE}📜 RULE${RESET} ${name.padEnd(20)} │ ${rule.tokens.toLocaleString().padStart(10)} tk │ ${pct.padStart(10)} % │ ${CostCalculator.formatCurrency(cost).padStart(10)} │`
      );
    }
    ...
    for (const hook of audit.hooks) {
      ...
      lines.push(
        `│ ${DIM}⚙️ HOOK${RESET} ${name.padEnd(20)} │ ${hook.tokensOverhead.toLocaleString().padStart(10)} tk │ ${pct.padStart(10)} % │ ${CostCalculator.formatCurrency(cost).padStart(10)} │`
      );
    }
    ...
    if (audit.anomalies.length > 0) {
      ...
      for (const a of audit.anomalies) {
        ...
        if (a.severity === 'CRITICAL') badge = `${BG_RED}${WHITE} CRITICAL ${RESET}`;
        else if (a.severity === 'INFO') badge = `${BG_BLUE}${WHITE} INFO ${RESET}`;
