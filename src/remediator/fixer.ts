...
import { AuditResult, FixOptions, FixResult } from '../types/index.js';
...
export class ConfigFixer {
   ...
   */
  static applyFix(audit: AuditResult, options: FixOptions = {}): FixResult {
    ...
    try {
      ...
      if (options.deduplicateSkills !== false) {
        ...
        for (const dup of duplicates) {
          if (dup.filePath && fs.existsSync(dup.filePath) && dup.source === 'workspace') {
            ...
            config.skills = config.skills.filter((s: any) => {
      ...
      if (options.disableBloatedMcp) {
        ...
        if (config.mcpServers) config.mcpServers = mcpServers;
        if (config.mcp_servers) config.mcp_servers = mcpServers;
      }
