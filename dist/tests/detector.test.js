...
test('AnomalyDetector should identify heavy MCP overhead and skill duplicates', () => {
    ...
    assert.ok(audit.sessionSummary.totalContextBeforeConversation > 40000, `Total tokens should be > 40k in bloated config, got ${audit.sessionSummary.totalContextBeforeConversation}`);
    assert.ok(audit.sessionSummary.overheadPercentage > 20, `Overhead percentage should be > 20%, got ${audit.sessionSummary.overheadPercentage}%`);
...
test('AnomalyDetector should recognize clean configuration', () => {
    ...
    const bundle = ConfigParser.parseAll(paths);
    const audit = AnomalyDetector.analyze(bundle);
    assert.ok(audit.sessionSummary.totalContextBeforeConversation < 15000, `Clean config should have low token count, got ${audit.sessionSummary.totalContextBeforeConversation}`);
    assert.ok(audit.sessionSummary.overheadPercentage < 10, `Overhead should be under 10%, got ${audit.sessionSummary.overheadPercentage}%`);
});
